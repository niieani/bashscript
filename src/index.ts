...
import {declareVariable, ast, referenceVar, statement, ASTObject, comment, declareFunction, print, extractedToRootScope, astGroup, TraverseScope, ScopeContext} from './writer/writer'
...
function unsupportedVisitor(node: AST.Node): VisitorReturn {
  return [
    ...
    ...node.getText()
      .split('\n')
      .map(line =>
        comment(`<unsupported> ${line.padEnd(40, ' ')} # ${node.getKindName()}`)
      ),
  ]
}
...
const getReduceVisitorReturns = (
  startWith: VisitorReturn = []
) => (
  list: VisitorReturn
) => list.concat(startWith)

const reduceVisitorReturns = getReduceVisitorReturns()
const getCommentObjects = (node: AST.Node) => getComments(node).map(text => comment(text, false))
...
const getNameGenerator = (name : string) =>
  <T>(
...
function functionVisitor(node: AST.FunctionDeclaration, rootScope = true): VisitorReturn {
  ...
  return rootScope
    ? functionStatements
    : ast`${
      extractedToRootScope(functionStatements)
      }${
      getNameFromScope(
...
function fileVisitor(node: AST.Node): VisitorReturn {
  switch (node.getKind()) {
    ...
    default:
      ...
      return list.getChildren().map((node) => nodeVisitor(node, true)).flatten(1)
  }
}
