...
function VariableDeclaration({
  ...
  const write = writer`declare ${anyFlags && '-'}${readonly && 'r'}${VariableDeclarationFlags[type]} ${name}${initializer && `=`}${initializer}`
}

const empty: Output = {here: '', root: ''}
const reduceOutput = (
  list: Array<Output>
) => list.reduce(({here: mergedHere, root: mergedRoot}, {here, root}) => ({
  here: `${mergedHere}${here}`,
  root: `${mergedRoot}${root}`,
}), empty)

const reduceRootOutput = (
  list: Array<Output>
) => list.reduce((reducedRoot, {root}) => `${reducedRoot}${root}`, '')

const writeNode = (node : Node | string | undefined | false) => typeof node === 'object'
  ? node.write()
  : {here: node || '', root: ''}

const writeNodes = pipe(
  map(writeNode),
  reduceOutput,
)
...
const writer = (strings : TemplateStringsArray, ...nodes : Array<Node | string | undefined | false>) => {
  const writtenNodes = nodes.map(writeNode)
  const root = reduceRootOutput(writtenNodes)
  const here = strings.map((part, index) => {
